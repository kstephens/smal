...
smal_buffer *smal_buffer_alloc()
{
  ...
  if ( (offset = smal_buffer_offset(addr)) ) {
    smal_debug(2, "offset %p = 0x%0lx", (void*) addr, (unsigned long) offset);
    free_addr = addr;
...
{
  assert(smal_buffer_ptr_is_validQ(self, ptr));
